  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.offer_date ? format(parseISO(record?.offer_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    {
      id: 'start_date',
      header: 'Start Date',
    ...
    hasAccess('applicant', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/applicants/view/${record.applicant?.id}`}>
